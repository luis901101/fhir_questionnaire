...
void main() {
  ...
  group('questionnaire extensions do not reach the response', () {
    test('when the questionnaire declares no expression at all', () async {
      ...
      final response = await controller.generateResponse(
      ...
      expect(
        items.map((item) => item.linkId.valueString),
      ...
      for (final item in items) {
        expect(
          item.extension_,
          isNull,
          reason: 'item ${item.linkId.valueString}',
        );
      }
    });

    test('on the siblings that follow a calculated item', () async {
      final questionnaire = Questionnaire.fromJsonString(
      ...
      expect(
        _responseItemOf(
          response,
          'calculated',
        ).answer!.first.valueBoolean?.valueBoolean,
        isTrue,
      );
    ...
    test('when an expression cannot be evaluated', () async {
      final questionnaire = Questionnaire.fromJsonString(
      ...
      expect(broken.answer!.first.valueBoolean?.valueBoolean, isFalse);
    });

    test(
      'leaving a signature item with only its signature extension',
      () async {
        final questionnaire = Questionnaire.fromJsonString(
        ...
        expect(
          groupResponse.extension_!.single.url.valueString,
